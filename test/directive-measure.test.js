...
const
	select = el => el.querySelectorAll('span'),
...
customElements.define(
	...
	component(({
		...
		return html`
			...
			<div measure="${ measure(select, onMeasure) }"><span>Text1</span><span>Text2</span><span>Text3</span></div>
		`;
	})
);
...
suite('measure', () => {
	test('collects initial measurements accurately', async () => {
		const onMeasure = spy();
		await fixture(html`<test-measure style="width: 300px" .select="${ select }" .onMeasure="${ onMeasure }"/>`);
		await nextFrame();
	...
	test('reports resizes accurately', async () => {
		const
			onMeasure = spy(),
			element = await fixture(html`<test-measure style="width: 300px" .select="${ select }" .onMeasure="${ onMeasure }"/>`);
